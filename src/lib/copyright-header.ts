...
export function ensureUpdatedCopyrightHeader(opts: Options): void {
  const files = collectFiles(opts.include);
...
export function collectFiles(include: ReadonlyArray<string>): ReadonlyArray<string> {
  const gitFiles = child_process.execSync('git ls-files', { encoding: 'utf8' }).split('\n');

  const includeRegexps = include.map(pattern => new RegExp(pattern));
